...
// configure esri-loader to use version 3.36 from the ArcGIS CDN
// NOTE: make sure this is called once before any calls to loadModules()
setDefaultOptions({ version: '3.36' })
```
...
| -- | -- | -- | -- |
| `version` | `string` | `'4.19'` | The version of the ArcGIS API hosted on Esri's CDN to use. |
...
// configure esri-loader to use version 3.36
// and the CSS for that version from the ArcGIS CDN
const options = { version: '3.36', css: true };
...
```html
<!-- index.html -->
<script src="https://js.arcgis.com/4.19/" data-esri-loader="loaded"></script>
```
